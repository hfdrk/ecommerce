...
export default function CheckoutPage() {
  ...
  if (items.length === 0 && !done) {
    return (
      <div className="mx-auto max-w-lg px-5 py-20 text-center sm:px-8">
        <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-[var(--background-alt)] text-[var(--muted-foreground)]">
  ...
  if (done) {
    return (
      <div className="mx-auto max-w-lg px-5 py-20 text-center sm:px-8">
        <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-[var(--accent-subtle)] text-[var(--accent)]">
          <svg width="32" height="32" viewBox="0 0 24 24" fill="none" aria-hidden>
    ...
    <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8 sm:py-14">
      ...
      >
        <div className="space-y-8">
          <section className="rounded-xl border border-[var(--border)] bg-white p-6 shadow-[var(--shadow-xs)] sm:p-8">
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              <label className="block text-sm font-medium text-[var(--foreground)]">
                ...
                <input required name="name" autoComplete="name" className="field-input mt-2" />
              </label>
              <label className="block text-sm font-medium text-[var(--foreground)]">
                <span className="text-[var(--muted-foreground)]">Email</span>
                <input required name="email" type="email" autoComplete="email" className="field-input mt-2" />
              </label>
            </div>
          </section>
          ...
          <section className="rounded-xl border border-[var(--border)] bg-white p-6 shadow-[var(--shadow-xs)] sm:p-8">
            ...
            <div className="mt-6 grid gap-4">
              <label className="block text-sm font-medium text-[var(--foreground)]">
                ...
                <input required name="line1" autoComplete="address-line1" className="field-input mt-2" />
              </label>
              <div className="grid gap-4 sm:grid-cols-3">
                <label className="block text-sm font-medium text-[var(--foreground)] sm:col-span-1">
                  ...
                  <input required name="city" autoComplete="address-level2" className="field-input mt-2" />
                </label>
                <label className="block text-sm font-medium text-[var(--foreground)] sm:col-span-1">
                  <span className="text-[var(--muted-foreground)]">State</span>
                  <input required name="state" autoComplete="address-level1" className="field-input mt-2" />
                </label>
                <label className="block text-sm font-medium text-[var(--foreground)] sm:col-span-1">
                  <span className="text-[var(--muted-foreground)]">ZIP</span>
                  <input required name="zip" autoComplete="postal-code" className="field-input mt-2" />
                </label>
              </div>
            </div>
          </section>
            ...
            {items.map(({ product, quantity }) => (
              <li key={product.id} className="flex justify-between gap-4">
                <span className="min-w-0 text-[var(--muted-foreground)]">
                  ...
                  <span className="tabular-nums text-[var(--muted-foreground)]">×{quantity}</span>
                </span>
